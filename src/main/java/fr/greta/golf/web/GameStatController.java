...
public class GameStatController {
    ...
    @GetMapping(path = "/fr/user/gameRate/selectCompetition")
    ...
    @GetMapping(path = "/fr/user/gameRate/collectData")
    public String gameRate(@RequestParam(name = "idCompet")Long id, Model model){
        Optional<Competition> competition = competitionRepository.findById(id);
        if (competition.isPresent()){
            model.addAttribute("competition", competition.get());
            model.addAttribute("course", competition.get().getCourse());
        }else {
            return "redirect:/fr/user/gameRate/selectCompetition";
        }
        return "forms/formGameRateData";
    }

    @PostMapping(path = "/fr/user/gameRate/collectData")
